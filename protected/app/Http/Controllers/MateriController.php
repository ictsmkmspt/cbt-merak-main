...
class MateriController extends Controller
{
  ...
  public function simpan_materi()
  {
    ...
    // id_kelas: kosong / null berarti tampil untuk semua kelas
    $id_kelas = Input::get('id_kelas') ?: null;
  ...
  public function get_materi()
  {
    ...
    $query = Materi::where('id_user', Auth::user()->id)
                   ->where('judul', 'LIKE', '%'.$q.'%');

    // Filter per kelas jika dipilih
    if ($id_kelas && $id_kelas != '') {
      $query->where(function($q2) use ($id_kelas) {
        $q2->where('id_kelas', $id_kelas)
           ->orWhereNull('id_kelas'); // tetap tampilkan yang untuk semua kelas
      });
    }

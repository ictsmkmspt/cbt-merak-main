...
<div class="col-sm-12 col-md-8 col-lg-8 dash-left">
  ...
  <div class="panel panel-announcement">
    ...
    <div class="panel-body">
      <h2>Materi</h2>
      <h4>Tulis materi pembelajaran dan tentukan kelas yang dapat mengaksesnya. Materi juga bisa disertai soal-soal latihan.</h4>
    </div>
  </div>
  <div class="panel panel-default">
    ...
    <div class="panel-body">
      <div class="row">
        <div class="col-md-12">
          <button type="button" class="btn btn-primary btn-md" id="btn-materi"><i class="fa fa-plus"></i> Tulis Materi</button>
          ...
          <div class="wells" style="margin: 15px 0 0 0; display: none; background: #fff" id="wrap-materi">
            <div class="form-horizontal" style="margin: 0; padding-bottom: 0 !important;">
              ...
              <div class="form-group">
                ...
                <div class="col-sm-10">
                  <textarea class="form-control" name="isi" id="isi" placeholder="Isi Materi" style="background: #fff"></textarea>
                </div>
              </div>
              <div class="form-group">
              ...
              {{-- ===== PILIH KELAS ===== --}}
              <div class="form-group">
                <label class="col-sm-2 control-label">Tampilkan ke Kelas</label>
                <div class="col-sm-10">
                  ...
                  <small class="text-muted"><i class="fa fa-info-circle"></i>
                    Pilih kelas tertentu agar materi hanya terlihat di kelas tersebut.
                    Biarkan <b>"Semua Kelas"</b> agar materi terlihat oleh semua kelas.
                  </small>
                </div>
              </div>
              {{-- ===== /PILIH KELAS ===== --}}
          ...
          {{-- ===== FILTER CARI + FILTER KELAS ===== --}}
          <div class="row" style="margin: 15px 0 5px 0">
            <div class="col-md-7">
              <input type="text" class="form-control" id="q" placeholder="Cari berdasarkan Judul (Enter)">
            </div>
            <div class="col-md-5">
          ...
          <div class="table-responsive" id="wrap-table-materi" style="margin-top: 10px">
            <table class="table">
              <thead>
                <tr>
                  <th>#</th>
              ...
              <tbody>
                ...
                <?php
                  if ($materi->status == 'Y') {
                    $status_label = "<span class='label label-primary'>Tampil</span>";
                  }else{
                ...
                <tr id="baris{{ $materi->id }}">
                  ...
                  <td>{{ $materi->judul }}</td>
                  <td style="width: 120px">{!! $nama_kelas !!}</td>
                  <td style="width: 75px">{!! $status_label !!}</td>
                  <td style="text-align: center; width: 120px">
                    <a href="{{ url('/materi/ubah/'.$materi->id) }}" data-toggle='tooltip' title="Ubah materi" class="btn btn-primary btn-xs"><i class="fa fa-pencil-square-o"></i></a>
                    <button data-toggle='tooltip' title="Hapus materi" class="btn btn-danger btn-xs btn-hapus-materi" data-id="{{ $materi->id }}"><i class="fa fa-trash"></i></button>
                    <a href="{{ url('/materi/detail/'.$materi->id) }}" data-toggle='tooltip' title="Detail materi" class="btn btn-success btn-xs"><i class="fa fa-eye"></i></a>
                  </td>
                </tr>
                @endforeach
                @else
                <tr><td colspan="5" class="alert alert-danger">Belum ada data untuk ditampilkan.</td></tr>
                @endif
              </tbody>
            </table>
            {!! $materis->render() !!}
...
<script type="text/javascript">
  ...
  (function ($) {
    function readyFn() {
      ...
      $("#q").keyup(function(e){
        if(e.keyCode == 13) loadMateri();
      });

      // Filter berubah langsung reload tabel
      ...
      $("#simpan").click(function(){
        ...
        var dataString = 'sesi='+sesi+'&judul='+judul+'&isi='+isi+'&status='+status+'&id_kelas='+id_kelas;
        $.ajax({
          type: "POST",
      ...
      // Hapus materi (event delegation untuk support ajax reload tabel)
      $(document).on('click', '.btn-hapus-materi', function(){
        var id = $(this).data('id');
        var row = '#baris'+id;
